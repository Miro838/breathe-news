...
$id = $name = $position = $bio = $profile_image_url = $social_links = "";
...
if (isset($_GET["id"]) && is_numeric($_GET["id"])) {
    ...
    $sql = "SELECT name, position, bio, profile_image_url, social_links FROM team WHERE id = ?";
    ...
    if ($stmt->num_rows == 1) {
        $stmt->bind_result($name, $position, $bio, $profile_image_url, $social_links);
        $stmt->fetch();
    } else {
        echo "<script>alert('No team member found with this ID.');</script>";
        exit();
    }
    $stmt->close();
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update team member in the database if no errors
    if (empty($name_err) && empty($position_err) && empty($bio_err) && empty($profile_image_err) && empty($social_links_err)) {
        $sql = "UPDATE team SET name = ?, position = ?, bio = ?, profile_image_url = ?, social_links = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $name, $position, $bio, $profile_image_url, $social_links, $id);
